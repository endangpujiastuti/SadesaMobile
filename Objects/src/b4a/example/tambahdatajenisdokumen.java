...
public class tambahdatajenisdokumen extends Activity implements B4AActivity{
	...
	public void onCreate(Bundle savedInstanceState) {
        ...
        mostCurrent = this;
		if (processBA == null) {
			processBA = new anywheresoftware.b4a.ShellBA(this.getApplicationContext(), null, null, "b4a.example", "b4a.example.tambahdatajenisdokumen");
			...
			if (p != null && p != this) {
                BA.LogInfo("Killing previous instance (tambahdatajenisdokumen).");
				p.finish();
			}
		}
	...
	private void afterFirstLayout() {
        if (this != mostCurrent)
			return;
		activityBA = new BA(this, layout, processBA, "b4a.example", "b4a.example.tambahdatajenisdokumen");
        ...
        if (BA.isShellModeRuntimeCheck(processBA)) {
			...
			processBA.raiseEvent2(null, true, "CREATE", true, "b4a.example.tambahdatajenisdokumen", processBA, activityBA, _activity, anywheresoftware.b4a.keywords.Common.Density, mostCurrent);
			_activity.reinitializeForShell(activityBA, "activity");
		}
        initializeProcessGlobals();		
        initializeGlobals();
        
        BA.LogInfo("** Activity (tambahdatajenisdokumen) Create, isFirst = " + isFirst + " **");
        processBA.raiseEvent2(null, true, "activity_create", false, isFirst);
		isFirst = false;
		if (this != mostCurrent)
			return;
        processBA.setActivityPaused(false);
        BA.LogInfo("** Activity (tambahdatajenisdokumen) Resume **");
...
@Override
 ...
 protected void onStop() {
    ...
    public static Class<?> getObject() {
		return tambahdatajenisdokumen.class;
	}
	...
	public void onPause() {
        ...
        if (!dontPause)
            BA.LogInfo("** Activity (tambahdatajenisdokumen) Pause, UserClosed = " + activityBA.activity.isFinishing() + " **");
        else
            BA.LogInfo("** Activity (tambahdatajenisdokumen) Pause event (activity is not paused). **");
        if (mostCurrent != null)
    ...
    private static class ResumeMessage implements Runnable {
		...
		public void run() {
            tambahdatajenisdokumen mc = mostCurrent;
